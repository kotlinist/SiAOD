...
vector<node*> expand(node* current) {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        new_node->num_path = current->num_path;
        (new_node->num_path).push_back(pair<int, int>(x, y));
//        cout << "(" << x << "; " << y << ") ";
...
int bfs(node* start) {
    ...
    while(!q.empty()) {
        ...
        setVisited(x->state);
        if (x->state == goal) {
            printf("Передвижения (0): ");
            for (int i = 0; i < x->path.size(); i++)
                printf("%c", x->path[i]);
            printf("\n");
            printf("Передвигаемые элементы: ");
            for (int i = 0; i < x->num_path.size(); i++)
                printf("%d ", x->state[x->num_path[i].first][x->num_path[i].second]);
            printf("\n");
            return 1;
        }
...
int main() {
    ...
    printf("Раскрыто нод: %d\n", expanded_nodes);
    cout << "Время (микросекунды): " << duration.count() << endl;
